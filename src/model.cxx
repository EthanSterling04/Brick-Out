          ...
          random_boost_source(-config.max_boost, config.max_boost)
{
    for (int r = 0; r < config.brick_rows; r++) {
        for (int c = 0; c < config.brick_cols; c++) {
            int x = config.side_margin + c * (config.brick_spacing.width + config.brick_dims().width);
            int y = config.top_margin + r * (config.brick_spacing.height + config.brick_dims().height);
            bricks.push_back(Block(x, y, config.brick_dims().width, config.brick_dims().height));
        }
    }
}
